...
class ChatGPTTelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.openai = openai
        self.commands = [
            BotCommand(command='help', description='დამხმარე მენიუ'),
            BotCommand(command='image', description='სურათის შექმნა *ინგ. ენაზეა* მაგ: /image Cat on the lion'),
        ]
        self.budget_limit_message = "TOKEN ERROR[404] CONTACT ADMINISTRATOR"
        self.usage = {}
        self.last_message = {}
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        help_text = 'გამარჯობა! მე ვარ Gakpo!' + \
                    '\n\n' + \
                    '\n'.join(commands) + \
                    '\n\n' + \
                    "მომწერე კითხვა და მე დაგეხმარები!"
    ...
    async def image(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if image_query == '':
            await context.bot.send_message(chat_id=chat_id, text='მიუთითეთ ფოტოს შინაარსი! (*მაგ image Cat on lion*)')
            return
